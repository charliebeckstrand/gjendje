...
Every app juggles `localStorage`, `sessionStorage`, URL params, and in-memory state — each with their own API. **gjendje** replaces it all with a single primitive. Choose where state lives. The rest is handled.
