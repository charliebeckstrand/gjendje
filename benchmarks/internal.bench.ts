...
const computedFanInSuite = defineSuite('computed-fan-in', {
	'Computed Fan-In (Many Dependencies)': (bench) => {
		// 5 deps
		const deps5 = Array.from({ length: 5 }, (_, i) =>
			state(uniqueKey(`fan5-${i}`), { default: i }),
		)
...
const lifecycleSuite = defineSuite('lifecycle', {
	'Instance Lifecycle (Create + Destroy)': (bench) => {
