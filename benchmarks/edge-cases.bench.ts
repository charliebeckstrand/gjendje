...
async function main() {
	...
	console.log('  Edge-Case Benchmark: subtle performance pain points')
	console.log('='.repeat(70))

	await benchComputedDiamond()
	await benchPeekVsGet()
	await benchCollectionWatch()
	await benchSnapshot()
	await benchCacheHit()
	await benchEffectLifecycle()
	await benchBatchWithComputed()
	await benchReadWriteInterleave()
	await benchHistoryLargeValues()
	await benchCollectionChaining()
	await benchShallowEqual()
	await benchWatchEnhancer()
	await benchGCPressure()
	await benchInterceptorRejection()
	await benchComputedStaleness()
