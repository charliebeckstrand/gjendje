...
describe('select', () => {
	it('derives a value from a single source', () => {
		const user = state('sel-basic', { default: { name: 'Jane', age: 30 }, scope: 'render' })
	...
	it('recomputes when source changes', () => {
		const counter = state('sel-recompute', { default: 1, scope: 'render' })
	...
	it('caches value between source changes', () => {
		const counter = state('sel-cache', { default: 1, scope: 'render' })
	...
	it('notifies subscribers on change', () => {
		const counter = state('sel-notify', { default: 0, scope: 'render' })
		const doubled = select(counter, (n) => n * 2)
	...
	it('skips notification when derived value is unchanged', () => {
		const counter = state('sel-skip', { default: 1, scope: 'render' })
	...
	it('participates in batch()', () => {
		const counter = state('sel-batch', { default: 0, scope: 'render' })
		const doubled = select(counter, (n) => n * 2)
	...
	it('can depend on a computed instance', () => {
		const a = state('sel-comp-a', { default: 2, scope: 'render' })
		const b = state('sel-comp-b', { default: 3, scope: 'render' })
	...
	it('auto-generates unique keys', () => {
		const s = state('sel-key-auto', { default: 0, scope: 'render' })
	...
	it('uses provided key', () => {
		const s = state('sel-key-custom', { default: 0, scope: 'render' })
	...
	it('has render scope', () => {
		const s = state('sel-scope', { default: 0, scope: 'render' })
		const derived = select(s, (n) => n)

		expect(derived.scope).toBe('render')
	})

	it('peek returns cached value', () => {
		const s = state('sel-peek', { default: 5, scope: 'render' })
	...
	it('destroy stops listening', () => {
		const counter = state('sel-destroy', { default: 0, scope: 'render' })
		const doubled = select(counter, (n) => n * 2)
